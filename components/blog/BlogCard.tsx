...
const BlogCard = ({
  ...
  return (
    <Link
      ...
      } gap-y-4 gap-x-6 rounded-xl ${className} hover:transition-transform cursor-pointer blog-card overflow-hidden relative hover:shadow-xl hover:scale-[1.02]! duration-300  `}
    >
      {post.coverImage && (
        <Image
          ...
          className={` rounded-xl object-cover aspect-video max-md:h-full`}
        />
      )}
      ...
      >
        <h3 className="text-[min(4vw,24px)] leading-tight tracking-wide font-bold">{post.title}</h3>
